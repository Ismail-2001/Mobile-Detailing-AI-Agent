...
// --- CHAT IP RATE LIMITER ---
// Backstop rate limiter keyed by IP address. While the session-based limiter
// (checkRateLimit) catches abuse per session ID, an attacker can bypass it by
// generating new session IDs on each request. This IP-level limiter prevents
// that by capping ALL chat requests from a single IP regardless of session.
// Limit: 30 requests per minute per IP (real users won't exceed ~5-10/min).
const chatIpCounts = new Map(); // ip -> { count, windowStart }
const CHAT_IP_WINDOW_MS = 60 * 1000;
const CHAT_IP_MAX_REQUESTS = 30;

export function checkChatIpRateLimit(ip) {
    const now = Date.now();
...
// For external webhooks (Meta, etc.): 60 requests/min per IP.
// Higher limit than chat because Meta sends delivery/read receipts too.
const webhookCounts = new Map();
const WEBHOOK_WINDOW_MS = 60 * 1000;
const WEBHOOK_MAX_REQUESTS = 60;

export function checkWebhookRateLimit(ip) {
    const now = Date.now();
    const entry = webhookCounts.get(ip);

    if (!entry || now - entry.windowStart > WEBHOOK_WINDOW_MS) {
        webhookCounts.set(ip, { count: 1, windowStart: now });
        return null;
    }
    ...
    if (entry.count > WEBHOOK_MAX_REQUESTS) {
        const retryAfterMs = WEBHOOK_WINDOW_MS - (now - entry.windowStart);
        return { retryAfterMs, retryAfterSec: Math.ceil(retryAfterMs / 1000) };
    }
