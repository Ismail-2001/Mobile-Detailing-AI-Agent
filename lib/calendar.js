...
export async function checkAvailability(date, requestedDuration = 120, businessId) {
    ...
    const bookedTimes = await getBookedTimesForDate(date, businessId);
...
/**
 * Query Supabase for all booked times on a given date, scoped to business.
 * Returns an array of time strings (e.g., ["8:00 AM", "2:00 PM"]).
 * Uses admin client to bypass RLS.
 */
async function getBookedTimesForDate(date, businessId) {
    ...
    try {
        ...
        // Scope to business if provided (multi-tenant)
        if (businessId) {
