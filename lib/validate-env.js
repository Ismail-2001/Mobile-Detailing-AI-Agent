...
export function validateEnv() {
    ...
    for (const [feature, config] of Object.entries(PAIRED_VARS)) {
        ...
        if (setVars.length > 0 && missingVars.length > 0) {
            ...
            console.error(`\n!!! PARTIAL CONFIGURATION: ${feature} !!!`);
            console.error(`  Set: ${setVars.join(', ')}`);
            console.error(`  Missing: ${missingVars.join(', ')}`);
            console.error(`  ${config.description}`);
            console.error(`  Setup: ${config.setupUrl}`);
            console.error(`  Both must be set together or the OAuth flow will fail.\n`);
